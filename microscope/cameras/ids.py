...
class IDSuEye(microscope.devices.TriggerTargetMixIn,
    ...
    Args:
        ...
        buffer_length (int): size of queue when acquiring images with
            triggers.  If `buffer_length` is less than or equal to
            zero, the queue size is infinite.
    """
    def __init__(self, serial_number: str = None,
                 buffer_length: int = 0) -> None:
    ...
    def _get_binning(self) -> Tuple[int, int]:
        ...
        return (_FLAG_TO_HORZ_BINNING[h_bin], _FLAG_TO_VERT_BINNING[v_bin])
...
def _get_info_of_all_cameras() -> Iterable[ueye.UEYE_CAMERA_INFO]:
    n_cameras = ctypes.c_int(0)
    if ueye.GetNumberOfCameras(ctypes.byref(n_cameras)) != ueye.SUCCESS:
        raise RuntimeError('failed to get number of cameras')
